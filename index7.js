  ...
  client.once("clientReady", async () => {
    ...
    if (activeGiveaways.length === 0) {
      ...
      console.log(
        `Resuming ${activeGiveaways.length} active giveaway(s) from giveaways.json:`,
      );
      for (const [messageId, g] of activeGiveaways) {
        const msRemaining = g.endsAt - Date.now();
        if (msRemaining <= 0) {
          console.log(
            `  - "${g.prize}" (${messageId}) — end time already passed while offline, ending now.`,
          );
          endGiveaway(client, messageId).catch(console.error);
        } else {
          console.log(
            `  - "${g.prize}" (${messageId}) — resuming, ${(msRemaining / 60000).toFixed(1)} min remaining.`,
          );
          setTimeout(
            () => endGiveaway(client, messageId).catch(console.error),
            msRemaining,
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      if (
        ...
        // Give Discord a moment to receive the reply above before the process exits.
        // If this bot is run under manager.js (or pm2, etc.), it will detect this
        // exit and start the process back up automatically. If you're running it
        // directly with `node index.js` with nothing supervising it, this will just
        // shut the bot down — you'd need to start it again by hand.
